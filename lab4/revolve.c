...
int main()
{
    ...
    const int midx = infoHeader.biWidth / 2;
    const int midy = infoHeader.biHeight / 2;
    ...
    for(int i = 0; i< infoHeader.biHeight ; i ++) {
        for(int j = 0; j < rowSize ; j ++ ) {
            ...
            if(x0 >= 0 && x0 < infoHeader.biWidth && y0 >= 0 && y0 < infoHeader.biHeight)
                newImageData[i * rowSize + j] = imageData[y0 * rowSize + x0];
            else
    ...
    // 写入文件
    char savePath[]="../lab4/ProcessedPic/revolve.bmp";
