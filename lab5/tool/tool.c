...
void makeHistogramAndNewImage(char *sourceFile,  char *histogramFile, char* imageFile, int T) {
    ...
    // 保存直方图文件
    FILE *f=fopen(histogramFile,"wb");
    if(f==NULL){
        perror("can not open file2!");
    return ;
    }
    // 保存新文件的图片信息
    FILE *f2=fopen(imageFile,"wb");
    if(f2==NULL) {
        perror("can not open file3!");
        return ;
    }
    ...
    fwrite(imageData, rowSize * infoHeader.biHeight, 1, f2);
